...
@objc class WaveLoadingIndicator: UIView {
    ...
    private let animationUnitTime = 0.15//重画单位时间, redraw unit time
    
//    private let heavyColor = UIColor(red: 38/255.0, green: 227/255.0, blue: 198/255.0, alpha: 1.0)
    private let heavyColor = UIColor(red: 0, green: 0, blue: 1, alpha: 1.0)
    ...
    private let progressTextFontSize: CGFloat = 12.0
...
//    var waving: Bool = false
    ...
    var waving:Bool = true {
        didSet {
            self.setNeedsDisplay()
        }
    }
    
    var titleText: NSString = "0" {
        didSet {
            self.setNeedsDisplay()
        }
    }
